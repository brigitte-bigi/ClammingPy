...
export class Reference {
    // CONSTANTS
    /**
     * Where a search for a publication is sent.
     *
     * A search is the only way to reach a publication that is always
     * available, whatever the BibTeX entry holds. This engine keeps no track
     * of who asks.
     */
    static SEARCH_URL = 'https://duckduckgo.com/?q=';
    ...
    /**
     * Build a search for this publication on the internet.
     *
     * A BibTeX entry may carry no address at all. A search is built from what
     * every entry has some of, its title and its first signatory, so that a
     * reader is never left without a way to reach the publication.
     *
     * Nothing is ever fetched here: the address is built and handed over, and
     * following it is the reader's decision.
     *
     * @returns {Link} A link that always exists.
     */
    searchLink() {
        const words = [this.field('title')];

        if (this.#authors.length > 0) {
            words.push(this.#authors[0].lastName);
        }

        const query = words.filter(word => word.length > 0).join(' ');

        return new Link(Reference.SEARCH_URL + encodeURIComponent(query));
    }
}

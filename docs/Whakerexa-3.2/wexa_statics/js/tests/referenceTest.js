...
// -----------------------------------------------------------------------
// A search on the internet is always available, built from the title and
// the first author.
// -----------------------------------------------------------------------

reference_tests.add_test(() => {
    const author = new Author(1, 'Brigitte', '', 'Bigi', '');
    const reference = build_reference({title: 'A Large Open Corpus'}, [author]);
    const search = reference.searchLink();

    UnitTest.assert_values_equals(LinkKind.SEARCH, search.kind(), "reference_search_link_kind_test");
    UnitTest.assert_array_contains(true, [search.address.includes('Large')],
        "reference_search_link_title_test");
    UnitTest.assert_array_contains(true, [search.address.includes('Bigi')],
        "reference_search_link_author_test");
});

// -----------------------------------------------------------------------
// A search is still built when the reference has no author at all.
// Requirement B4.
// -----------------------------------------------------------------------

reference_tests.add_test(() => {
    const reference = build_reference({title: 'A Large Open Corpus'});
    const search = reference.searchLink();

    UnitTest.assert_values_not_equals('', search.address, "reference_search_link_without_author_test");
});

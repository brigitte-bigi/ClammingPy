...
bibliography_table_tests.add_test(() => {
    ...
    UnitTest.assert_values_not_equals(null, table.querySelector('caption'), "table_caption_test");
...
bibliography_table_tests.add_test(() => {
    ...
    const contents = row.querySelectorAll('.bib-disclosure-content');
    UnitTest.assert_values_equals(true, contents[0].hidden, "table_content_hidden_test");
    UnitTest.assert_array_contains(true, [row.textContent.includes('What the paper says.')],
...
// -----------------------------------------------------------------------
// Every address of a reference is reachable, and a search is always
// there, whatever the entry holds. Requirement B12.
// -----------------------------------------------------------------------
...
bibliography_table_tests.add_test(() => {
    ...
    UnitTest.assert_values_equals(3, with_links.length, "table_links_test");
    UnitTest.assert_values_equals('http://www.example.org/paper.pdf', with_links[0].getAttribute('href'),
        "table_link_address_test");
    UnitTest.assert_values_equals(1, without.length, "table_search_link_always_test");
});
...
bibliography_table_tests.add_test(() => {
    ...
    UnitTest.assert_values_not_equals(french.querySelector('caption').textContent,
        english.querySelector('caption').textContent, "table_language_test");
});

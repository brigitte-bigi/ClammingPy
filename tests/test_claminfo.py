...
class TestClamInfo(unittest.TestCase):
    def test_clam_info_creation(self):
        ...
        clam_info = ClamInfo("nothing")
        self.assertEqual(clam_info.name, "nothing")
        self.assertEqual(clam_info.args, [])
        self.assertEqual(clam_info.source, "")
        self.assertIsNone(clam_info.docstring)

        # Fill in only the fields with a wrong type
        with self.assertRaises(TypeError):
            ClamInfo(["wrong"])
        with self.assertRaises(TypeError):
            ClamInfo("wrong", args={"1": "args1"})
        with self.assertRaises(TypeError):
            ClamInfo("wrong", source=1)
        with self.assertRaises(TypeError):
            ClamInfo("wrong", docstring=1)
...
class TestClamInfoMarkdown(unittest.TestCase):
    ...
    def test_constructor(self):
        c = ClamInfoMarkdown(self.clam_info)
        self.assertIsInstance(c, ClamInfoMarkdown)
        with self.assertRaises(TypeError):
            ClamInfoMarkdown("def toto(): pass")
